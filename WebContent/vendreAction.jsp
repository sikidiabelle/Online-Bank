...
<%@ page import="model.Actionnaire" %>
<%@ page import="dao.ActionDAO" %>

<!-- Definition du titre de la page -->
<% session.setAttribute("title_page", "Vendre des actions"); %>
...
<%} else{//Si user connecté
	...
	<%}else{ //Si user connecté a un compte titre %>
		...
		<div class="col-8">
			<h1 class="text-center">Vendre mes actions</h1>
			...
			String numCpt = cptDAO.getNumCptTitre(id);
			ArrayList<Actionnaire> listAct = actDAO.getListActionnaire(numCpt);
			Action a;
			if (listAct.size() == 0) { //Si user n'a pas d'actions dans son compte %>
				<div class="row bg-light p-3 border border-dark">
					<a class="text-danger mx-auto" href="acheterAction.jsp">
						Vous n'avez pas d'actions associés à ce compte! Achetez-en
					</a>
				</div>				
			<% } else { // Si user a des actions dans son compte
			
			for (Actionnaire act : listAct){
				a = actDAO.getAction(act.getIdAction());
			%>
				
				<form action="VenteTreat" method="post">
					<div class="row bg-light p-3 border border-dark">
						<!-- Envoi de l'id actionnaire pour le traitement -->
						<input type="hidden" name="idActionnaire" value=<%= act.getId() %> >
						
						<!-- Affichage infos des actions -->
						<div class="col-3">
							<h4 class="text-center"><%= a.getNom() %></h4>
						</div>
						<div class="col-3">
							<h4 class="text-center"><%= a.getPrix() %></h4>
						</div>
						<div class="col-3">
							<h4 class="text-center"><%= act.getQuantite() %></h4>
						</div>
						<div class="col-3">
							<input type="submit" class="btn btn-dark btn-block btn-lg" value="Vendre">
						</div>
						
					</div>
				</form>
				
			<% } //En si user a des actions dans son compte
			} //End for actions %>
